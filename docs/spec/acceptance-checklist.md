...
- **Combo tees seeded 2026-08-22** (Green/Black, Black/Silver, Silver/Gold on Red/Blue/Black — migration `20260822090000_seed_combo_tees.sql`). All nine carry the card's rating/slope/total AND full per-hole yardages: the card's single ▲/▼ "Combo" row gives all three combos (▲ = back tee of the pair, ▼ = forward tee), and every combo's 18 yardages were verified to sum to its printed total. So they pass the course-publish check like any base tee. **Bone Valley's seven tees (incl. its three combos) followed on 2026-09-09** — see the Bone Valley card section at the end.
- **Bone Valley `year_opened` (2025) and its placeholder tee `par` (72)** were working values — the schema forces non-null on both. **Superseded 2026-09-09** by `20260909120000_seed_bone_valley_card.sql` (year 2026, real card, published).
...
## Bone Valley card — seeded and published (2026-09-09, branch `bone-valley-card`)

| Item | Evidence |
|---|---|
| Real card transcribed from Kyle's cart photos (front grid + USGA rating panel) | `20260909120000_seed_bone_valley_card.sql`: 18 pars (36/36 = 72), men's SI (1–18 permutation), 7 tees with rating/slope/total, 126 hole yardages. Photo resolution 632×485 — every row was cropped and 4× enlarged to read; each base tee's yardages sum to the printed Out/In/Total |
| Combo tees included (Kyle: "make sure to include the combo tees") | Green/Black 6875, Black/Silver 6320, Silver/Gold 5430 derived from the ▲/▼ row `▼▼▲▲▼▼▲▼▲ / ▼▲▲▼▲▲▲▼▼` (▲ = back tee), all three reconcile to the printed totals. Two ambiguous Black cells (16, 17) were fixed by that reconciliation: 185/365 is the only reading consistent with Black In = 3190 and all three combos |
| Published | The seed re-runs the publish RPC's checks in a `do` block and flips `data_is_placeholder` only when they all pass. Local DB after `supabase db reset`: `data_is_placeholder = false`, 7 tees, par 3s on 3/7/12/16, 126 non-null yardages |
